...
app.get('/setEX', (req, res) => {
  res.cookie('expirable', 'esta es la cookie de ejemplo', { maxAge: 30000 })
...
app.get('/setSigned/:nombre', (req, res) => {
  const nombreCookie = req.params.nombre;
  res.cookie(nombreCookie, `${nombreCookie} valor`, { signed: true })
  res.send('Set Signed Cookie')
})
